...
if __name__ == "__main__":
    ...
    trans_conv1 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=[2,1], padding=1, output_padding=[1,0])
    x = torch.randn(1, 1, 156, 128)  # Input tensor with 3 channels, 16x16 spatial dimensions
    y = trans_conv1(x)
    print(y.shape)
    trans_conv2 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=[2, 1], dilation=[2,1],padding=1,
                                     output_padding=[1, 0])
    y = trans_conv2(y)
    print(y.shape)