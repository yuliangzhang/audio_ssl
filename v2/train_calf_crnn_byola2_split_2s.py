...
from byol_a2.CRNN import CRNN
from byol_a2.cnn14_decoupled import Cnn14_Decoupled
...
class BYOLALearner(pl.LightningModule):
    ...
    def __init__(self, cfg, model, tfms, local_tfms, **kwargs):
        ...
        local_model = AudioNTT2022(n_mels=cfg.n_mels, d=cfg.feature_d)
        self.local_learner = BYOL(local_model, image_size=cfg.shape_local, **kwargs)
        self.lr = cfg.lr
        self.tfms = tfms
    ...
    def training_step(self, wavs, batch_idx):
        ...
        loss_global, _, _ = self.global_learner(global_frame1.transpose(1,2), global_frame2.transpose(1,2))
        loss_local, _, _ = self.local_learner(local_frame1, local_frame2)


        # lms_batch = self.pre_norm(lms_batch)
        # sub_lms_batch = self.pre_norm(sub_lms_batch)
        # Original globa and local embeddings extraction
        global_frames_embeddings = self.global_learner.online_encoder.net(lms_batch.transpose, get_embeddings=True)
        local_embeddings = self.local_learner.online_encoder(sub_lms_batch)
...
def main(config_path='config_calf_cnn14_split_1s.yaml', d=None, epochs=None, resume=None) -> None:
    ...
    learner.calc_norm_stats(dl, n_stats=100)
